...
export const removeDisabledScenes = (
  ...
  const sceneTypesList = engineSchema?.sceneTypes || defaultSceneTypesList;
  const enabledSceneTypeIds = sceneTypesList
...
const compile = async (
  ...
  const activeSceneTypes = engineSchema?.sceneTypes || defaultSceneTypesList;
  const unusedSceneTypeIds = difference(
    activeSceneTypes.map((st) => st.key),
    usedSceneTypeIds,
  );
  ...
  const enabledSceneTypeIds = activeSceneTypes
    .map((sceneType) => sceneType.key)
    .filter((key) => !disabledSceneTypeIds.includes(key));
  for (
    let sceneIndex = 0;
  ...
  const usedSceneTypes = (engineSchema?.sceneTypes || defaultSceneTypesList).filter((type) =>
